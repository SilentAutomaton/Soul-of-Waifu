...
REASONING_EFFORT_TO_BUDGET = {'low': 2000, 'medium': 6000, 'high': 16000, 'xhigh': 32000}
VALID_EFFORT_LEVELS = {'low', 'max', 'xhigh', 'high', 'medium'}
...
class AnthropicProvider(BaseAIProvider):
    ...
    def _build_payload(self, messages: list[dict], kwargs: dict, default_max_tokens: int, stream: bool, thinking_variant: str, tools=None) -> dict:
        ...
        payload = {'model': self.model, 'messages': anthropic_messages, 'stream': stream}
        if system_text:
                ...
                try:
                    async with client.stream('POST', self.base_url, headers=headers, json=payload) as response:
                        if response.status_code != 200:
                            ...
                            yield f"\n⚠️ Anthropic API Error: {response.status_code}"
                            return
    ...
    async def generate(self, messages: list[dict], **kwargs) -> dict:
        ...
        logger.error(f'Anthropic API Error: all thinking variants failed: {last_error}')
        raise RuntimeError(f'Anthropic API Error: all thinking variants failed ({last_error})')